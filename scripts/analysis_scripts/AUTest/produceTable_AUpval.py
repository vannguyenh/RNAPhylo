...
SUFFIX_IGNORE = '_consel'  # suffix before .pv for ignore_pseudo
...
def main():
    ...
    print(df.head())
    full_csv = join(DIR_AU, '260204_AU_pValues_bothpVals_SEED.csv')
    df.to_csv(full_csv, sep='\t')

    df_table = df.pivot(index='RNA_family', columns='Model', values='p_RNA_raw').sort_index()
    print(df_table.head())
    out_csv = join(DIR_AU, '260208_AUpvalues_RNApVal_SEED.csv' )
