...
# extract all tip labels (tokens before ":<number>")
...
# drop odd numeric prefixes and underscores; keep token before first "_"
canon <- function(x) {
  x <- sub("^_*[0-9]+\\.?[0-9]*_", "", x)
  x <- sub("^_*", "", x)
  sub("^([^_]+).*", "\\1", x)
...
# fetch scientific names from NCBI taxonomy for a set of taxids
...
# Build accession -> preferred label map using RFAM tips:
# 1) use [taxid] → NCBI scientific name,
# 2) if no taxid: try a simple species guess (last chunk before '['),
# 3) last resort: keep the original label
build_label_map_from_rfam <- function(rfam_tips) {
  ...
  # preferred labels via taxonomy
  sci_by_taxid <- fetch_scientific_names(taxids)
  
  guess_species <- function(lbl) {  # minimal fallback if no taxid
    lb <- regexpr("\\[", lbl, perl=TRUE)[1]
    s <- if (lb == -1) lbl else substr(lbl, 1, lb-1)
    # keep chunk after last "/" then after first "_" within that
    s <- sub("^.*?/", "", s)
    s <- sub("^[^_]*_", "", s)
    s <- gsub("_+", " ", s)
    s <- sub("\\.+$", "", s)
  ...
  # Reduce to one label per accession (first occurrence wins)
  # If the same accession appears multiple times, they’ll get the same name.
...
# Relabel a Newick text using accession -> name map
# (disambiguate duplicates by appending [ACC] or [taxid] only when needed)
relabel_newick <- function(newick_text, acc2name, acc2taxid = NULL,
                           disambig_tag = c("acc","taxid")) {
...
# Find outgroup tip by prefix (works if label became "Eptesicus fuscus [..]")
...
# For plotting: shared depth and per-panel right padding by label width
...
## ---- 2) Build map from RFAM tips (taxid → scientific name) ----
rfam_tips <- tip_labels_from_newick(rfam_txt)
lut <- build_label_map_from_rfam(rfam_tips)
acc2name  <- lut$acc2name
acc2taxid <- lut$acc2taxid  # used only if you pick disambiguation by taxid

## ---- 3) Relabel all three trees with scientific names ----
...
# Write relabeled trees + mapping (audit)
rfam_out <- file.path(out_dir, sub("\\.newick$|\\.nwk$|\\.tre$", ".labeled.newick", basename(rfam_path), ignore.case = TRUE))
...
write.csv(data.frame(accession = names(acc2name), scientific_name = unname(acc2name),
